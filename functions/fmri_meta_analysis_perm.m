...
function outStructure = fmri_meta_analysis_perm(InputData)
    ...
    for i = 1:size(InputData.dat, 1)
            ...
            voxelWiseResults = random_effects_meta_analysis_perm(InputData.dat(i,rowInd)', InputData.covariates(rowInd));
            ...
            statImageZ.dat(i, 1) = voxelWiseResults.Z_statistic;
            %statImageVariance.p(i, 1) = voxelWiseResults.Q_pvalue;
            %statImageVariance.N(i,1) = voxelWiseResults.df+1;
            
            % fmri_data objects cause problems when values are exactly 0.
    ...
    outStructure.Zimage = statImageZ;
