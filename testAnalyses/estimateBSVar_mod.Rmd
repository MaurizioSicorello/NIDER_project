...
pacman::p_load(tidyverse, purrr, oro.nifti, neurobase, mvmeta, metafor, ggridges, cowplot, NeuRRoStat)
...
maskedVox <- filter(distrAllT, ROImask != 0)
...
dataG <- database %>% as.tibble() %>% 
  ...
  arrange(V3) %>% 
  mutate(hedgeG = NeuRRoStat::hedgeG(t = Tvalue, N = V3, type = 'exact'),
         varG = NeuRRoStat::varHedgeT(g = hedgeG, N = V3))
...
OrderFactorSS <- database %>% as.tibble() %>%
  mutate(study = factor(paste('S', 1:nstud, sep = ''),
            levels = paste('S', 1:nstud, sep = ''))) %>%
            select(-c(1:2)) %>% arrange(V3)
dataG$study <- factor(dataG$study, levels = rev(OrderFactorSS$study))
ggplot(dataG, aes(y = study, x = hedgeG)) +
  geom_density_ridges2(aes(fill = V3), scale = 2) +
  scale_y_discrete('Study', expand = c(0.01, 0)) +
  scale_x_continuous("Hedges' g", expand = c(0.01, 0)) +
  scale_fill_gradient2('Sample size', 
                       low = '#3288bd',
...
voxID <- dataG %>% mutate(voxel = rep(1:sum(ROI != 0), nstud))
